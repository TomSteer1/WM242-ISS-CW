...
conn.execute(PRAGMA)
conn.execute("CREATE TABLE IF NOT EXISTS files (id uuid PRIMARY KEY, userid TEXT, filename TEXT, public BOOL DEFAULT FALSE, mimetype TEXT)")
...
def files():
    ...
    return render_template('files.html', publicFiles=list_public_files(), user=user, files=list_user_files(user['id']))
...
def getFile(fileId):
    ...
    user = getUser(session['token'])
    if dbFile[3] is False and dbFile[1] != user['id']:
        return "File not found",404
    key = get_key(fileId)
    print(key)
...
def uploadFile():
    ...
    conn = sqlite3.connect('database.db')
    conn.execute(PRAGMA)
    conn.execute('INSERT INTO files (id,userid,filename,public,mimetype) VALUES (?,?,?,?,?)',(fileID,getUser(session['token'])['id'],file.filename,public,file.mimetype))
    conn.commit()
    conn.close()
